...
function get_microsite() {
    $microsite = \Illuminate\Support\Facades\Cache::remember('microsite',6400, function () {
        return \App\Microsite::All();
    });
...
function get_cookie($name) {
    return $_COOKIE[$name] ?? $_GET[$name] ?? null;
}
...
function check_microsite() {
    ...
    $value = null;
    foreach($microsite as $site):
        if($site->slug == $path[1] && $path[1] !='id' ){
            $value = $site;
        }
        if(!empty($path[2]) && $site->slug == $path[2] && $path[1] =='id' ){
            $value = $site;
        }
    endforeach;
...
function get_static_option($key,$default = null)
{
    ...
    if(in_array($key,['home_page_variant','navbar_variant','site_logo']) && !empty(check_microsite())) {
        return check_microsite()->$key;
    } else {
...
function filter_static_option_value(string $index , array $array = []){
    if(in_array($index,['home_page_variant','navbar_variant','site_logo']) && !empty(check_microsite())) {
        return check_microsite()->$index;
    } else {
        return $array[$index] ?? '';
    }
}
