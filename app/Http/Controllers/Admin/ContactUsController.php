...
{
    const BASE_PATH = 'backend.contact-us.';
    ...
    public function index(){
        $all_contact = TheNoveContactUs::orderBy('id','desc')->paginate(10);
        return view(self::BASE_PATH.'contact-us',compact('all_contact'));
    }

    public function delete(Request $request,$id){
        TheNoveContactUs::find($id)->delete();
        return redirect()->back()->with(NexelitHelpers::item_delete());
    }
    public function bulk_action(Request $request){
        TheNoveContactUs::whereIn('id',$request->ids)->delete();
        return response()->json(['status' => 'ok']);
    }
    public function pagination(Request $request,$filter = null) {
        if($request->ajax()) {
            $all_contact = TheNoveContactUs::orderBy('id','desc');
            if(!empty($filter)) {
                $all_contact = TheNoveContactUs::where('name','like','%'.$filter.'%')
    ...
    public function export(Request $request,$type,$filter = null) {
        $all_contact = TheNoveContactUs::orderBy('id','desc')->get();
        if(!empty($filter)) {
            $all_contact = TheNoveContactUs::where('name','like','%'.$filter.'%')->orwhere('phone','like','%'.$filter.'%')->orderBy('id','desc')->get();
        }

        if($type == 'excel') {
            $format = \Maatwebsite\Excel\Excel::XLSX;
            return Excel::download(new ContactUsExport($filter),'contact-us-the-nove-'.date('d-m-Y').'-'.$filter.'.xlsx', $format);
        } else if($type == 'csv') {
            $format = \Maatwebsite\Excel\Excel::CSV;
            return Excel::download(new ContactUsExport($filter),'contact-us-the-nove-'.date('d-m-Y').'-'.$filter.'.csv', $format);
        } else {
            return response($all_contact->toJson())
                ->withHeaders([
                    'Content-Type' => 'text/plain',
                    'Cache-Control' => 'no-store, no-cache',
                    'Content-Disposition' => 'attachment; filename="contact-us-the-nove-'.date('d-m-Y').'-'.$filter.'.json',
                ]);
        }
