...
class GlobalVariableMiddleware
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        $microsite = check_microsite();
        if(!empty($microsite)) {
            if(!is_array((json_decode($microsite->menu_id, true)))) {
                $primary_menu = Menu::where(['id' => $microsite->menu_id])->first();
            } else {
                $menu_id = json_decode($microsite->menu_id, true)[$lang];
                $primary_menu = Menu::where(['id' => $menu_id])->first();
            }
        } else {
