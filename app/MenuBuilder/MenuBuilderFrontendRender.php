...
class MenuBuilderFrontendRender
{
    ...
    private function get_attribute_string(array $li_attributes):string
    {
        ...
        foreach ($li_attributes as $attr => $value){
            ...
            if (!empty($value) && $attr != 'class'){
                $value = is_array($value) ? $value['class'][0] : $value;
                $attr_val .= $attr.'="'.$value.'"';
            }elseif($attr === 'class'){
                if (!is_array($value)){
                    $attr_val .= $attr.'="'.$value.'"';
    ...
    {
        $parent_class = get_static_option('home_' . get_static_option('home_page_variant') . '_menu_parent_class','menu-item-has-children');
        $active_class = '';
        if(\Illuminate\Support\Facades\Route::currentRouteName() != 'homepage' && get_static_option('navbar_variant') != '07') {
            $parent_class = 'menu-item-has-children';
        }
        $attributes_string = property_exists($menu_item,'children') ? ['class' => [$parent_class]]  : [];
        ...
        if ($ptype === 'custom'){
            ...
            if (request()->path() === ltrim($menu_item->purl,'/')){
                $active_class = 'active';
    ...
    protected function render_children_item($menu_item,$default_lang){
        if (empty((array)$menu_item)){return;}
        $child_class = get_static_option('home_' . get_static_option('home_page_variant') . '_menu_child_class','sub-menu');
        if(\Illuminate\Support\Facades\Route::currentRouteName() != 'homepage' && get_static_option('navbar_variant') != '07') {
            $child_class = 'sub-menu';
        }
        $output= '';
        $output .= '<ul class="'.$child_class.'">'."\n";
