...
class BlogSliderNuvasaStyleTwo extends PageBuilderBase
{
    ...
    public function admin_render()
    {
        ...
        $output .= Number::get([
            ...
            'info' => __('enter how many item you want to show in viewport'),
        ]);
        $output .= Number::get([
            'name' => 'total_items',
            'label' => __('Total Item'),
    ...
    public function frontend_render()
    {
        ...
        foreach ($blogs as $item){
            ...
            $category_markup .= <<<HTML
                <div class="item my-2 px-3">
                    <div class="blog_container container_{$i}">
                        <a href="{$route}">
        ...
        ";
        $fetchRoute = route('frontend.blog.fetch.blog',$category);
        $js .= <<<JS
            ...
            $(document).on('click', '#blog_section{$random} nav a', function(event){
                event.preventDefault(); 
                let page = $(this).attr('href').split('page=')[1];
                $(document).find('#blog_section{$random} #overlay').show()

                $.ajax({
                    url:"{$fetchRoute}?item={$slider_items}&order_by={$order_by}&order={$order}&page="+page,
                    success: function(result) {
                        let section = $(document).find("#blog_part{$random}");
                        $.each(result, function(index,data){
                            let container = section.find('.container_'+index)
                            container.find('a').attr("href", data.route)
                            container.find('.blog_name h5').html(data.title)
                            container.find('.blog_date p').html(data.date)
                            container.find('.blog_image').html(data.image)
                            container.find('.blog_desc p').html(data.excerpt)
                            container.find('.blog_link a').attr("href", data.route)
                        })
                        $(document).find('#blog_section{$random} nav').html(result.links)
                        $(document).find('#blog_section{$random} #overlay').hide()
                    }
                })
            });
        ...
        return <<<HTML
            ...
            <section class="blog_section" id="blog_section{$random}" style="padding-bottom:{$padding_bottom}px;padding-top:{$padding_top}px">
                <div id="nuvasabay">
    ...
    {
        return __('Nuvasa Blog Slider: 02');
    }
}