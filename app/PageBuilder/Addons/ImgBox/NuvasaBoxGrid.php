...
class NuvasaBoxGrid extends PageBuilderBase
{
    protected $a = 1;
    use RepeaterHelper;
    /**
    ...
    {
       return 'service/nuvasa-grid-01.png';
    }
    ...
    private function render_slider_markup(int $index)
    {
        ...
        if(!empty($image['img_alt'])) {
            $image_markup = render_image_markup_by_attachment_id($image,$class);
        } else if(!empty($title)) {
            $image_markup = render_image_markup_by_attachment_id($image,$class.'" alt="'.$title.' cover');
        } else {
            $image_markup = render_image_markup_by_attachment_id($image,$class.'" alt="'.$label.'');
        }

        $logo = $this->get_repeater_field_value('logo', $index, LanguageHelper::user_lang_slug());
        
        if(!empty($logo['img_alt'])) {
        ...
        return <<<HTML
            <div class="col-lg-6 col-md-12 col-sm-12 pb-5 pt-1 pt-lg-0">
                <div class="overflow-hidden my-3 position-relative">
                    <div class="position-relative {$div_class}">
                        {$faicon}
                        {$image_markup}
                    </div>
