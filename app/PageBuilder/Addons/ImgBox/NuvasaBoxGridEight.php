...
class NuvasaBoxGridEight extends PageBuilderBase
{
    protected $a = 1;
    use RepeaterHelper;
    /**
    ...
    {
        return __('Nuvasa Box Grid: 01 (3 column)');
    }

    private function render_slider_markup(int $index)
    {
        $settings = $this->get_settings();
        $image_class = 'residential_bg_img';
        ...
        if(empty($title) && empty($description)) {
            $image_class = 'discover-img';
        }

        if(!empty($image['img_alt'])) {
            $image_markup = render_image_markup_by_attachment_id($image,$image_class);
        } else if(!empty($title)) {
            $image_markup = render_image_markup_by_attachment_id($image,$image_class.'" alt="'.$title);
        } else {
            $image_markup = render_image_markup_by_attachment_id($image,$image_class.'" alt="'.$label);
        }
        ...
        return <<<HTML
            <div class="col-lg-4 col-md-12 col-sm-12 pb-5 pt-5 pt-lg-0">
