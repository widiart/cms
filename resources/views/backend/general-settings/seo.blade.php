                                ...
                                @foreach($all_languages as $key => $lang)
                                    <div class="tab-pane fade @if($key == 0) show active @endif" id="nav-home-{{$lang->slug}}" role="tabpanel" aria-labelledby="nav-home-tab">
                                        ...
                                        <div class="form-group margin-top-20">
                                            <label for="about_page_{{$lang->slug}}_meta_tags">{{__('About Meta Tags')}}</label>
                                            <input type="text" name="about_page_{{$lang->slug}}_meta_tags"  class="form-control" data-role="tagsinput" value="{{get_static_option('about_page_'.$lang->slug.'_meta_tags')}}" >
                                        </div>
                                        <div class="form-group">
                                            <label for="about_page_{{$lang->slug}}_meta_description">{{__('About Meta Description')}}</label>
                                            <textarea name="about_page_{{$lang->slug}}_meta_description"  class="form-control" rows="5" >{{get_static_option('about_page_'.$lang->slug.'_meta_description')}}</textarea>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
