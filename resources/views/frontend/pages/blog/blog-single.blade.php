...
@section('content')
    ...
    @else
        @include('frontend.pages.blog.blog-single-22')
    @endif
@endsection
...
@section('scripts')
    ...
    @if(in_array(get_static_option('home_page_variant'),['22','23']))
        <script>
            function myFunction() {
                navigator.clipboard.writeText('{{ url()->full() }}').then(function () {
        ...
        <script>
            $('.share-single a').on('click', function() {
               $.ajax({
                    url: "/ajax/sharebutton",
                    type: "POST",
                    dataType: "JSON",
                    data:{
                        _token: "{{ csrf_token() }}",
                        news_id: "{{ $blog_post->id }}" 
                    },
                    success: function(response) {
                        console.log(response)
                        $('.sharenumber').html(response);
                    },
